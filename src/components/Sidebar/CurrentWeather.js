...
import raindrop from "../../icons/rain-drops.svg";

function CurrentWeather() {
  const { weatherData, loading, isCelcius } = useMainContext();

  // Dynamically import the weather icon within the component function
  const weatherIcon = loading
    ? require(`../../icons/${weatherData.iconId}.svg`)
    : null;
  return loading ? (
    <div>
      ...
      <figure className="flex items-center h-5vh w-5vw justify-center">
        {/* Check if weatherIcon is not null before rendering */}
        {weatherIcon && (
          <img className="" src={weatherIcon} alt={weatherData.description} />
        )}
      </figure>
